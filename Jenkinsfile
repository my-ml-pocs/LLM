    ...
    stages {
            ...
            steps {
                script {
                    if (fileExists('helm/values.yaml')) {
                        ...
                        error "Neither helm/values.yaml nor deployment.yaml found. Deployment failed."
                    }
                }
            }
        }
        ...
        stage('Rollback Mechanism') {
            steps {
                script {
                    try {
                        // Simulate deployment verification (replace with actual verification logic)
                        sh 'echo "Verifying deployment..."'
                    } catch (Exception e) {
                        echo "Deployment failed. Rolling back to previous version."
                        sh '''
                        helm rollback ${APP_NAME} || echo "No previous Helm release to rollback to."
                        '''
                    }
                }
            }
